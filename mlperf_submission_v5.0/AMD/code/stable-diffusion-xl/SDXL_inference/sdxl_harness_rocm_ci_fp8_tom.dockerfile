...
RUN git clone --depth 1 -b sdxl_micro https://github.com/nod-ai/shark-ai.git \
  && cd shark-ai/ \
  && echo "IN SHARK AI" \
  && python3.11 -m pip install -r requirements.txt \
  && echo "INSTALLED REQUIREMENTS" \
  && python3.11 -m pip install -e sharktank/ shortfin/ \
  && echo "INSTALLED SHORTFIN"
...
RUN git clone https://github.com/nod-ai/SHARK-ModelDev -b bump-punet-fp8-model-tom \
  && cd SHARK-ModelDev \
  && python3.11 -m pip install --pre --upgrade -e models -r models/requirements.txt \
  ...
  && python3.11 -m pip uninstall iree-compiler iree-runtime iree-base-compiler iree-base-runtime -y
